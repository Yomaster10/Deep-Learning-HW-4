...
class PNMI_FGSM(Attack):
    def __init__(
        ...
        super(PNMI_FGSM, self).__init__(model, criterion, test_criterion, norm, data_shape,
                                  sample_window_size, sample_window_stride,
                                  pert_padding)

        self.alpha = alpha # learning rate
        self.mu = mu # decay factor (momentum)
    ...
    def PNMI_FGSM_gradient_ascent_step(self, pert, grad_t, data_shape, data_loader, y_list, clean_flow_list, alpha, eps, mu, device=None):
        theta = pert + alpha*mu*grad_t
        theta = self.project(theta, eps)
        theta_expand = theta.expand(data_shape[0], -1, -1, -1).to(device)

        grad_t_plus_one = torch.zeros_like(pert, requires_grad=False)
        for data_idx, data in enumerate(data_loader):

            dataset_idx, dataset_name, traj_name, traj_len, img1_I0, img2_I0, intrinsic_I0, img1_I1, img2_I1, intrinsic_I1, img1_delta, img2_delta, \
            ...
            # Where g is actually caclulated
            grad = self.calc_sample_grad(theta_expand, img1_I0, img2_I0, intrinsic_I0,
                                         img1_delta, img2_delta,
            ...
            with torch.no_grad():
                grad_t_plus_one = mu*grad_t + (grad / torch.norm(grad, p=1))
            ...
            with torch.no_grad():
                pert += alpha * torch.sign(grad_t_plus_one)
                pert = self.project(pert, eps)

        return pert, grad_t_plus_one

    def perturb(self, data_loader, y_list, eps, device=None, eval_data_loader=None, eval_y_list=None):
        alpha = self.alpha  # learning rate
        mu = self.mu # decay rate     

        print("Computing PNMI-FGSM attack with parameters:")
        ...
        data_shape, dtype, eval_data_loader, eval_y_list, clean_flow_list, eval_clean_loss_list, traj_clean_loss_mean_list, \
            clean_loss_sum, best_pert, best_loss_list, best_loss_sum, all_loss, all_best_loss = self.compute_clean_baseline(data_loader, y_list, eval_data_loader, eval_y_list, device=device)
        ...
        for rest in tqdm(range(self.n_restarts)):
            ...
            grad_t = torch.zeros_like(pert, requires_grad=False)
            for k in tqdm(range(self.n_iter)):
                print(" Attack optimization epoch: " + str(k))
                iter_start_time = time.time()

                # custom method for the gradient descent step
                pert, grad_t_plus_one = self.PNMI_FGSM_gradient_ascent_step(pert, grad_t, data_shape, data_loader, y_list, clean_flow_list, alpha, eps, mu, device=device)
                #pert = self.project(pert, eps)
                grad_t = grad_t_plus_one # iteration
                ...
                new_folder = 'temp/adv_current_pert_alpha='+ str(alpha) + '_mu=' + str(mu)
                try: 
                    os.mkdir(new_folder) 
                except OSError as error: 
                    print(error)
                img_path = new_folder + '/iter=' + str(k) + '.png'
                save_image(pert.detach()[0], img_path)
